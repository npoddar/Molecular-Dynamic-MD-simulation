...
int main(){
	...
	//dumpAtoms = "atom.restart.8200000";
	//restartFile = "restart.8200k.data";
	//newRestartFile = "restart.new.8200k";
...
void molecules(char *atomFile){
	...
	while( (fgets(line, 200000, input) != NULL ) ) {
		...
		if(scan == 10){
			if( (ycoor < ylow) || (ycoor > yhigh) || (xcoor < xlow) || (xcoor > xhigh) || (zcoor < zlow) || (zcoor > zhigh) ) {
